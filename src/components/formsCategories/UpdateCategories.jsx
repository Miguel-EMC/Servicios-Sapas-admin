...
const UpdateCategories = () => {
  ...
  const handleInputChange = (e) => {
    ...
    setCategorieData({ ...categorieData, [name]: value });
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://127.0.0.1:8000/api/categories/${id}/`, categorieData);
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  {categorieData.thumbnail ? (
                    <Avatar alt="Thumbnail" src={categorieData.thumbnail} />
                  ) : (
                    <Avatar>
                      <ImageIcon />
                    </Avatar>
